...
export const Narasingdi = (props) => {
    const[products,setProducts]=useState([])
    const[success,setSuccess]=useState(false)
    

    useEffect(()=>{
        fetch('https://sellinbd.com/api330088/product/searchbyPlace.php?place=Narasingdi&page_number=1&item_count=1000')
        .then(res=>{
            if(res.status===200){
                setSuccess(true)
                res.json()
    ...
    return (
        <section  className="category">
            ...
            <Row className="justify-content-center" >
                ...
                {
                    products.map(product=><AgriculturalTool
