...
export const Chandpur = (props) => {
    const[products,setProducts]=useState([])
    const[success,setSuccess]=useState(false)
    

    useEffect(()=>{
        fetch('https://sellinbd.com/api330088/product/searchbyItem.php?item=Agricultural Tools and Machinery&page_number=1&item_count=1000')
        .then(res=>{
            if(res.status===200){
                setSuccess(true)
                res.json()
    ...
    // },[])
    // useEffect(()=>{
    //     fetch('https://sellinbd.com/api330088/product/searchbyPlace.php?place=Chandpur&page_number=1&item_count=1000')
    //     .then(res=>{
    //         if(res.status===200){
    //             setSuccess(true)
    //             res.json()
